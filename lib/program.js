...
Program.prototype.setupCommands = function setupCommands () {
  ...
  files.forEach(function (path) {
    ...
    // returns "/" instead of "\"
    // and lower-cased drive varter
    path = path.replace(/\//g, separator)
               .replace(/^./, function ($1) {
                  return $1.toUpperCase()
                })
               .replace(join(self.path, 'commands'), '')
               .replace(separator, '');
    
    // include only .js files
    if (/\.js$/.test(path)) {
      var name = path.split(separator)
                     .join(self.delimiter)
                     .replace('.js', '');
      
      self.commands[name] = join(self.path, 'commands', path);
    }
  });
};
...
Program.prototype.run = function run () {
  ...
  var matches = names.filter(function (name) {
    ...
    name.split(self.delimiter).forEach(function (part, i) {
...
Program.prototype.help = function help () {
  ...
  var delimiters = max(Object.keys(this.commands).map(function (key) {
    return key.split(self.delimiter).length
  }));
