...
class Program {
  ...
  setupCommands () {
    ...
    files.forEach(path => {
      ...
      if (/\.js$/.test(path)) {
        ...
        let command = require(join(this.path, 'commands', path));
    ...
    this._level = max(Object.keys(this.commands).map(key => key.split(this.delimiter).length));
  }
  ...
  run () {
    ...
    process.on('uncaughtException', err => {
      let nativeErrors = [
  		...
  		// throw it
  		if (nativeErrors.indexOf(err.name) > -1) this.stdout.write(err.stack + os.EOL);
    ...
    // and command name from argv
   	let end = this.delimiter === ' ' ? this._level : 1;

   	process.argv.splice(0, 2 + end);

   	let name = argv.splice(0, end).join(this.delimiter);
  ...
  autoupdate (done) {
    ...
    exec('npm info ' + name + ' version', (err, latestVersion) => {
      ...
      let updateAvailable = semver.gt(latestVersion, version);
  ...
  help () {
    ...
    let names = Object.keys(this.commands).filter(name => {
      ...
      return !parentExists || parts.length < this._level;
    });
    ...
    let commands = names.map(name => {
      let desc = new this.commands[name]().help('compact');
